...
def sl_2_layer(z_position:float, layer_num:int, loop, grid_lines, grid_nodes,
    ...
    while layer_num < layer_end :
        ...
        grid_nodes += f"        {nodes_a}"
        cluster_a += f" {layer_num + 1}"
        grid_lines += f" {z_position:.3f}d0"

        z_position += thickness_b
        grid_nodes += f"        {nodes_b}"
        cluster_b += f" {layer_num + 2}"
        grid_lines += f" {z_position:.3f}d0"
...
def sl_multi_layer(z_position:float, layer_num:int, loop, grid_lines, grid_nodes,
               grid_factors, *thickness_cluster):
    """
    完整的超晶格生成函数。如果超晶格只有两层，且不需要梯度渐变，可以用简化版的sl_2_layer
    :param z_position:上一层的结束坐标
    :param layer_num:上一层的结束层数
    :param loop:超晶格循环数
    :param thickness_cluster: 依次写入每层的厚度和归属cluster,渐变厚度用(float,float)或者[float,float]的形式输入初始厚度和结束厚度
    因为每层有两个参数(厚度和cluster），这部分输入数量必须为偶数！
    :return:返回新的当前Z坐标，层数，lines,nodes,factors，最后依次返回每个cluster
    """
    if not len(thickness_cluster) % 2 == 0:
        print("请检查输入参数数量！")
        return
    units_num = len(thickness_cluster) // 2

    thickness_i = []
    cluster_i = []
    i:int = 0
    while i < (units_num * 2):  # 遍历输入的循环体结构,拆解成厚度和cluster两个列表
        thickness_i.append(thickness_cluster[i])
        i += 1
        cluster_i.append(thickness_cluster[i])
        i += 1
    ...
    for N in range(loop) :
        for i in range(units_num): #两层循环，总层数 = units_num * loop
            ...
            if current_layer is not None:
                ...
                grid_nodes += f"        {tools.get_nodes(current_layer,config.GRID_THICKNESS_SL)}"
                cluster_i[i] += f" {layer_num + 1}"
                grid_lines += f" {z_position:.3f}d0" #输出时缩短为3位小数，但z_position本身还是原始精度继续运算
                grid_factors += f"      1d0"
                layer_num += 1
    print(f"当前超晶格顶端位置为{z_position:.3f}，是第{layer_num}个region")
...
if __name__ == "__main__":
    ...
    former_z,former_n,grid_lines,grid_nodes,grid_factors,cluster1,cluster2 = sl_multi_layer(former_z, former_n,
    ...
    sl_a = 9.39
    sl_b = 2.45
    former_z, former_n, grid_lines, grid_nodes, grid_factors, cluster1, cluster3 = sl_multi_layer(former_z, former_n,
    ...
    #裁剪
    if len(grid_lines) > 300:
        grid_lines,grid_nodes,grid_factors,cluster1,cluster2,cluster3 = tools.merge_and_split(grid_lines, grid_nodes, grid_factors, cluster1, cluster2, cluster3)
    #加盖（nextnano默认会多一个region,这里直接扔进cluster1)
    cluster1 += f" {former_n+1}"
